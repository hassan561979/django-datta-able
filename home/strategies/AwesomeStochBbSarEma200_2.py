...
class AwesomeStochBbSarEma200_2:
        ...
        if self.order_type == -1:  # sell
            ...
            if ema_signal['signal'] == 1:
                ...
                if psar_signal['signal'] == 1:
                    ...
                    if bb_signal['signal'] == 1:
                        signals = {'signal': 1,
                                   'log': 'bb: {}'.format(bb_signal) + ' ema_200: {}'.format(ema_signal)}
                    else:
                        stoch = AwesomeStoch(df)
                        stoch_signal = stoch.get_signals()
                        if stoch_signal['signal'] == 1:
                            signals = {
                                'signal': 1, 'log': 'awesome_stoch: {}'.format(stoch_signal) + ' ema_200: {}'.format(ema_signal)}
