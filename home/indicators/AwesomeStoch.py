...
class AwesomeStoch:
    ...
    def generate_signals(self):
        ...
        stoch = ta.stoch(data['high'], data['low'], data['close'], k=self.stoch_k, d=self.stoch_d,smooth_k=self.smooth_k)
      
        # Generate signals
        stock_k=stoch['STOCHk_{}_{}_{}'.format(self.stoch_k,self.stoch_d,self.smooth_k)]
        stock_k=stock_k.iloc[-1]

        stock_d=stoch['STOCHd_{}_{}_{}'.format(self.stoch_k,self.stoch_d,self.smooth_k)]
        stock_d=stock_d.iloc[-1]

        if stock_k < self.stoch_oversold and stock_d < self.stoch_oversold and stock_k > stock_d:
           signal=1  
        ...
        self.signals = {'close':data['close'].iloc[-1],'stock_k':stock_k,'stock_d':stock_d,'signal':signal}
